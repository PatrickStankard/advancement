...
Advancement.prototype.getStreamAssets = function(stream, callback) {
  var browser, count, output, parseStreamAssets;
  ...
  parseStreamAssets = function() {
    ...
    if (retry === true) {
      ...
      if (count <= 10) {
        return browser.wait(500 * count, parseStreamAssets);
      }
    }

    browser.close();

    callback(null, retry === true ? null : output);
  };

  browser.visit(stream.href, parseStreamAssets);
};
